...
//dd($latestSession);
            ...
            // Dispatch the login job
            LogJob::dispatch($Auth, 'Login to the system');

            // Return the login email message view
...
dd($e->getMessage());
        ...
        try{
           ...
           // $user1 = User::where('email', $username->email)->firstOrFail();
            ...
            if($user->email){
               ...
               //  $minutesDifference += $difference->s; 
			...
			if ($latestSession) {
 			DB::table('sessions')
    			...
    			->delete(); // Only get the 'id' column for deletion
			//dd($sessionsToDelete);
			}
    ...
    public function userAccount()
    {
        ...
        return view('admin.system-config.user-account', compact('users','UserType','auth'));
    }
    ...
    public function userAccess()
    {
        ...
        return view('admin.system-config.user-access', compact('ListOfEmailAdd','auth'));
    }
    public function companyProfiles()
    {
       ...
       //dd($profiles);
        return view('admin.system-config.company-profile', compact('profiles', 'accounts','auth'));
        } catch (\Throwable $e) {
        ...
        try{
            if($id!=''){//UPDATE
                ...
                $validated = $request->validate([
                    ...
                    'Account_Manager_Email'=>'required|email|unique:users,email',
                ]);
            if($validated){
    ...
    public function apiAccounts()
    {
        ...
        $auth = Auth::user();
        $accounts = ApiAccount::all();
        return view('admin.system-config.account', compact('accounts','auth'));
    }
    ...
    {
        $auth = Auth::User();
        return view('admin.system-config.SystemConfig',compact('auth'));
    }
    public function ApiAccess()
    {
        ...
        return view('admin.system-config.ApiAccess', compact('options','auth')); //this a blade name
    }
